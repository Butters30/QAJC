...
import HomeWork8.DataBase.DatabaseRepository;
import HomeWork8.DataBase.DatabaseRepositorySQLiteImpl;
        ...
        if (periods.equals(Periods.FIVE_DAYS)) {
            ...
            for (WeatherResponse weather: weatherResponseList) {
                System.out.println("В городе " + ApplicationGlobalState.getInstance().getSelectedCity()
                        ...
                        " ожидается такая погода: Минимальная температура "
                        + weather.getTemperature().getMinimum().getValue() + "°С. Максимальная температура " +
                        weather.getTemperature().getMaximum().getValue() +
                        "°С. Днём - " + weather.getDay().getIconPhrase() + ". Ночью - "
                        + weather.getNight().getIconPhrase() + ".");

                WeatherData weatherData = new WeatherData(
                        ApplicationGlobalState.getInstance().getSelectedCity(),
                        weather.getDate().substring(0,10), weather.getDay().getIconPhrase(),
                        weather.getNight().getIconPhrase(),
...
//    @Override
//    public void getWeatherFiveDays(Periods periods) throws IOException {
//        String cityKey = detectCityKey();
//        if  (periods.equals(Periods.FIVE_DAYS)) {
//            HttpUrl url = new HttpUrl.Builder()
//                    .scheme("http")
...
//            int firstIndexBody = response.indexOf("[{\"Date\"");
//            int lastIndexBody = response.lastIndexOf("}");
//            response = response.substring(firstIndexBody, lastIndexBody);
//
//            ObjectMapper responseMapper = new ObjectMapper();
//            List<HomeWork8.WeatherResponse> weatherResponseList = responseMapper.readValue(response, new TypeReference<List<WeatherResponse>>() {
//            });
//
//System.out.println("------------------------------------");
//for (WeatherResponse weather : weatherResponseList){
...
//    }
//
//    public void getWeatherDay(Periods periods) throws IOException {
//        String cityKey = detectCityKey();
//        if  (periods.equals(Periods.NOW)) {
//            HttpUrl url = new HttpUrl.Builder()
//                    .scheme("http")
